...
void GraphicsComposerHwcTest::getDisplayProperty() {
    ...
    ASSERT_LT(0, width);
    ASSERT_LT(0, height);
    std::cout << "display config = " <<config  << "  width = " << width << "  height = " << height << std::endl;
    }
}
...
void GraphicsComposerHwcTest::getActiveConfig() {
//    std::vector<Config> configs;
	unsigned long config = 0 ;
    config = mComposerClient->getActiveConfig(config);
    ...
    std::cout << "active mode  = " << config << " width = " << mDisplayWidth << "height = " << mDisplayHeight << std::endl;
}
...
void GraphicsComposerHwcTest::getColorModes() {
     ...
     for (auto mode:modes) {
		 switch ((int)mode)
		 case 0:
				std::cout <<" color mode = ColorMode::NATIVE " << std::endl;
     }
...
int GraphicsComposerHwcTest::readBmp(unsigned char** image,char* fileName){
    ...
    if (mDisplayWidth != width || mDisplayHeight != height) {
        ...
        printf( "picture %d*%d size is not match display %d*%dsize\n ",width,height,mDisplayWidth,mDisplayHeight);
        return 0;
    }
    level = 4;
...
void GraphicsComposerHwcTest::displayStripePicture() {
    mWriter->selectDisplay(mPrimaryDisplay);
    mComposerClient->setPowerMode(mPrimaryDisplay, IComposerClient::PowerMode::ON);
    mComposerClient->setColorMode(mPrimaryDisplay, ColorMode::NATIVE);
    const native_handle_t* handle;
    ...
    if (strcmp(argv[1],"-s") == 0 && strcmp(argv[2],"display") == 0 && strcmp(argv[3],"mode")==0) {	
	 ...
	 while ((ch = getopt(argc,argv,"a:bcdesg"))!= -1) {
		...
		if (strcmp(argv[2],"color") == 0&& strcmp(argv[3],"mode")==0 ) {
		    mHwc.getColorModes();
		} else if (strcmp(argv[2],"active") == 0 && strcmp(argv[3],"mode") == 0) {
		    mHwc.getActiveConfig();
